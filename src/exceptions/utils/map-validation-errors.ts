  ...
  return errors.flatMap((error) => {
    ...
    if (!error.contexts || !Object.entries(error.contexts).length) {
      ...
      return {
        ...
        message:
          validationError ||
          VALIDATION_ERROR_CONTEXT.DEFAULT_VALIDATION_ERROR.message,
      };
    }

    return Object.entries(error.contexts).map(
      ([fieldName, { errorCode, message }]) => ({
        errorCode:
          errorCode ||
          VALIDATION_ERROR_CONTEXT.DEFAULT_VALIDATION_ERROR.errorCode,
        message:
          message ||
          error.constraints[fieldName] ||
          VALIDATION_ERROR_CONTEXT.DEFAULT_VALIDATION_ERROR.message,
      }),
    );
  });
